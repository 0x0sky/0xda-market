...
-- The base currency's rate is fixed: 1 USDT per unit, written by the core.
WITH base_price AS (
  INSERT INTO market.product_prices (sku, amount_usdt, source)
  VALUES ('usdt', 1, 'core')
  RETURNING id, created_at
)
UPDATE market.products AS product
SET current_price_id = base_price.id,
    current_price_usdt = 1,
    price_updated_at = base_price.created_at
FROM base_price
WHERE product.sku = 'usdt';
